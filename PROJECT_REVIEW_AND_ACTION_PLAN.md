...
- one blocking TypeScript issue prevents the mobile typecheck from completing;
...
### 2. Harden Supabase queries and error handling

- Priority: P0
- Quick win: Yes
- Estimated time: 1–2 hours
- Evidence: the current helpers in the Supabase utility layer do not inspect response errors and silently ignore failures.
- Recommended fix:
  - check `error` from Supabase responses before returning data;
  - throw meaningful errors for UI layers;
  - wrap network calls in a shared service layer so the app can show consistent loading/error states.

### 3. Fix the missing-data assumptions in the prop detail loader
...
### 4. Add environment and configuration validation for Supabase
...
### 5. Synchronize accessory toggles with prop changes

- Priority: P1
- Quick win: Yes
- Estimated time: 1–2 hours
- Evidence: the accessory card stores local state (`hasBag`, `hasKeyring`, `hasDisplayPlaque`) initialized from the prop props, but it does not resync when the parent prop updates after a refresh or mutation.
- Recommended fix:
  - derive the toggle state from the latest prop data, or
  - sync the local state in a `useEffect` whenever `prop.accessories` changes.

### 6. Reset and reinitialize forms correctly when switching between create/edit modes

- Priority: P1
- Quick win: Yes
- Estimated time: 1–2 hours
- Evidence: the prop and note forms use a form state initialized from `selectedProp`/`selectedNote`, but they do not fully reset when the selected entity changes or after a successful save.
- Recommended fix:
  - reset the form explicitly on route changes and after mutations;
  - use `reset()` from React Hook Form when entering create/edit modes.

### 7. Add explicit loading, error, and empty states for list/detail screens
...
### 8. Improve chart robustness and lifecycle handling
...
### 9. Reduce stale UI after mutations and invalidations
...
### 11. Introduce a shared API/result layer and remove duplicate data logic
...
### 12. Improve performance for long lists and search-heavy screens
...
### 13. Add offline awareness and better local persistence
...
### 14. Add observability and crash reporting
...
### 15. Add a picture gallery for prop details
...
### 16. Add grouping, ordering, and drag-and-drop for collection display
...
### 17. Add import/export for collection data
...
### 18. Add cloud sync and profile features
...
### 19. Add richer stats and price-history views
...
### 20. Add a full accessories inventory dashboard
